...
class _HistoryViewState extends ConsumerState<HistoryView> {
  ...
  Future<void> _onAddTaskPressed() async {
    final historyVm = ref.read(historyProvider.notifier);
  ...
  Future<void> _onDeleteTaskPressed(int taskId, String title) async {
    ...
    final shouldDelete = await showDialog<bool>(
      ...
      builder: (dialogContext) {
        return AlertDialog(
          ...
          content: Text(
            'Are you sure you want to delete "$title" from history?',
          ),
          actions: [
            TextButton(
              onPressed: () {
    ...
    final ok = await historyVm.deleteTask(taskId);
    ...
    if (!ok) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(state.errorMessage ?? 'Failed to delete task'),
        ),
      );
      return;
    }
  ...
  Widget build(BuildContext context) {
    ...
    final vm = ref.read(historyProvider.notifier);
